// Import crypto polyfill first to ensure it's available before any other imports
import './common/utils/crypto-polyfill';
...
async function bootstrap() {
  ...
  logger.log(`🚀 Application is running on: ${process.env.API_URL || 'http://localhost:3000'}`);
  if (process.env.NODE_ENV !== 'production') {
    logger.log(`📚 API Documentation: ${process.env.API_URL || 'http://localhost:3000'}/api/docs`);
  }
}
bootstrap();
