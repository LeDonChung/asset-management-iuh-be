...
export class AssetsController {
  ...
  @ApiResponse({ status: 200, description: 'RFID tag updated successfully', type: AssetResponseDto })
  @ApiResponse({ status: 404, description: 'Asset not found' })
  @ApiResponse({ status: 400, description: 'Bad request - Only Fixed Assets can have RFID tags' })
  async updateRfidTag(
    @Param('id') id: string,
  ...
  @ApiResponse({ status: 404, description: 'Asset not found' })
  @ApiResponse({ status: 400, description: 'Bad request - Asset does not have RFID tag' })
  ...
  @ApiOperation({ 
    ...
    description: 'Import tài sản từ file Excel với cấu trúc 13 cột (A-M). Tự động chia thành batch 5-10 dòng một lần để xử lý. Bao gồm cả RFID tag cho tài sản cố định.'
  })
  ...
  @ApiResponse({ status: 201, description: 'Assets imported successfully', type: ImportResultDto })
  @ApiResponse({ status: 400, description: 'Bad request - Invalid file format, data, or quantity limit exceeded' })
